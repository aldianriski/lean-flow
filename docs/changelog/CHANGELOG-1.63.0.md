...
# lean-flow — Changelog v1.63.x (rotated)

> Rotated verbatim from the root `CHANGELOG.md` at the **v1.65.0 release** (2026-09-13). §11 keeps the
> current and previous minor inline, so cutting v1.65.0 pushed v1.63.x out. This rotation was **overdue
> by one minor** — v1.64.0 was written at the SPRINT-098 close and never bumped, so no release event
> fired to trigger it; the SPRINT-100 promote governance review caught it. Reachable only from the
> root `CHANGELOG.md`.
