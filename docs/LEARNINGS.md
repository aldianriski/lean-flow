...
- seen: Sprint-074 (T2, hard-coded-rule-list seed reddening exactly the 2 rule-source cases; unsigned-as-proof seed reddening 4) · Sprint-075 (T6/T3, 10 breaks seeded on the ownership suite — 10 discriminated, 2 of them by exposing real defects)
- count: 2
- promoted: no
...
- **Second sighting, SPRINT-075 T6/T3 — the method worked, and then needed a guard of its own.** Ten breaks seeded against the new ownership suite; all ten reddened, and **two of them found real defects rather than confirming the suite**: the ADR exemption went unnamed on an ADR-only repo (the note sat after an early return, so 27 skipped files reported as "no documents found"), and `owner-role-must-match-whole-value` proved *nothing* — its fixture used `owner: Alice, Maintainer`, and a longer string is never a substring of a shorter role line, so the substring break it was named for could not redden it. The guard `-x` actually provides is against a **prefix** (`Main` vs `Maintainer`); fixture changed, break now reddens. **The addition to the method:** three seeded patches whose `sed` never matched reported the suite green, and *a green run behind a patch that never landed is not evidence* — it is indistinguishable from a suite that discriminates. So a seeding harness must verify the seed **landed** (`cmp` against the pristine copy) and report `SEED-ERROR`, not a pass, when it did not. The discrimination pass needs its own discrimination check. Also confirmed the restore leg matters: one seeding run was killed by a timeout mid-pass and left a break in the engine, caught only by an explicit `sha1sum` comparison afterwards. seen Sprint-074 · Sprint-075, count 2.
---
...
- count: 1
- promoted: **yes → `.claude/CLAUDE.md` § Behavioral Guidelines, the cross-check clause** (SPRINT-075 promote) — promoted at count 1 as the *second grain* of L-130's rule rather than on its own recurrence, because the two are one failure and splitting them across two homes would reproduce the gap that let this one through. The clause names both.
...
- count: 2
- promoted: **yes → `.claude/CLAUDE.md` § Behavioral Guidelines, the cross-check clause** (SPRINT-075 promote). Placed there rather than in a skill red-flag by §10's placement test: the flows that can hit this are *promote* (authoring a DoD), *task-decomposer* (a premise entering at intake) and any skill writing an acceptance threshold — and L-136's sighting entered via `TODO.md` at intake, a path a `lean-doc-generator` red-flag would never have reached. The promotion carries **both grains** — a figure and a structural claim — because they are one failure, and states *why* the existing clause did not fire: authoring feels like planning, not querying. Body stays here until the next promote's §11 collapse.
