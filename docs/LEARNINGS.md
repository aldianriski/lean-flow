...
## L-113 [tags: process] [status: active]: **A rule can be correctly placed, currently in context, and still not fire — because the enumeration of flows that trigger it omits the moment it is being *exercised*.** L-108 ("a guard is matched by shape, not by substring") was promoted into `.claude/CONTEXT.md` § Gates at SPRINT-061, placed by §10's test against an honest list of triggering flows: checker authoring · the night-run reaper · close-time log sweeps · fixture naming and symptom greps. In SPRINT-062 it was loaded in context for an entire session and reached **none** of three fresh violations, all on the corpus it was written about, all failing green: a `grep -c "promoted: yes"` whose target string §11's own collapse consumes; an `awk` scan with broken escaping that reported 29 of 30 entries as defective; a fixed-string search that named the one entry lacking a bold wrapper as a genuine gap. Two of the three were produced *while verifying the first*, and the worst was caught only because it contradicted a count taken minutes earlier — an accident, not a check. The placement was not wrong; the enumeration was incomplete in a specific and predictable way. Every listed flow is an **authoring** flow — writing a checker, naming a fixture, sweeping a log — while all three failures were **ad-hoc verification greps run inside a governance/gate pass**, which is not a flow anyone thinks to enumerate because it has no artifact and no name. It is also the worst place to omit: a verification result is acted on immediately, with no review step between the query and the conclusion. When placing a rule, enumerate not only the flows that *author* the thing it guards but the flows that *check* it — and treat "I am currently verifying something" as a trigger point in its own right. Distinct from L-099 (a rule its reader cannot read: here the reader could and did) and from L-105 (a rule evaluated at the wrong moment: here the moment was right and the rule simply was not consulted).
- seen: Sprint-062 · Sprint-064
- count: 2
- promoted: no
- **Second sighting, Sprint-063/064 — and it held under a deliberate test.** Five more L-108-class failures with the rule loaded, then SPRINT-064 T2 acted on this diagnosis directly: sorting all eleven instances by *the flow that was running* showed **8 of 11 were ad-hoc verification queries inside a governance pass** — the flow L-108's enumeration omitted, exactly as this entry predicted. The corrective was **not** another sentence in the same section: § Gates was demonstrably in context for all eight. What this sighting adds beyond the first is the *shape of the fix* — a rule that must fire at the moment of acting converts into a **required second query**, not a caution to recall, because every instance ever caught was caught by a disagreeing number rather than by remembering anything. Promotable at the next promote (count 2).
- related: L-108 (the rule that did not fire; count 6) · L-099 (a rule its reader cannot read) · L-105 (correct rule, wrong moment) · L-058 (the silent false negative this produces) · TASK-200
