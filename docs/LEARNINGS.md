...
## L-100 [tags: process] [status: active]: **A `Layers:` declaration frozen at promote cannot name the files implementation invents — treat it as a live declaration corrected per task, not a guess to be defended.** Six corrections across five tasks in SPRINT-055, every one the same cause: T1 created `check-count-claims.sh` (the count block was inline and path-bound, so it *had* to be extracted before any fixture could point at it), T2 created `check-epic-archive.sh` and moved a file to a path that did not exist at promote, T3 created a checker and touched `gen-index.sh`, T4 and T6 each created a checker, and T6 additionally edited `TODO.md` as task work. None of these were nameable when the Plan was written, because each was discovered by attempting the task. Leg 15 (`check-layers-observed.sh`) caught all six, so nothing escaped — the friction is the correction loop, not misses. The prior instance is recorded: TD-022 notes SPRINT-042 T3 defeated leg 14 the day it shipped by creating a file its own DoD prose never named. The move is to stop treating a mid-sprint `Layers:` edit as a failure of planning that needs justifying, and start treating it as the expected cost of a declaration written before the work — log it, declare it, continue. Cheap when expected; a recurring irritation when read as a mistake each time.
- seen: Sprint-042, Sprint-055
- count: 2
- promoted: no
...
- **Sprint-055 (recurrence, one sprint later).** T3's Plan scope said "pruning the 25 existing `docs/research/` files" and its `assumes:` line repeated the figure. Measured at the task's design step: **27 files, of which 26 are `status: current` and exactly one is `superseded`**. Both halves of the premise were wrong at authoring — the count, and the implied accumulation of spent verdicts that justified the task. The number was written into the Backlog entry at decompose, carried verbatim into the frozen Plan at promote, and signed at G2 without anyone running `ls | wc -l`. Nothing had changed in between; it was never measured. Caught only because `class: decision` forces a design step before writing, which re-derived it — the same escape route T3's own retention rule now depends on. Note the aggravator L-097 already names, firing again: `docs/research/` has no cap check, so its size is only ever known when someone counts.
- seen: Sprint-054, Sprint-055
- count: 2
- promoted: no
