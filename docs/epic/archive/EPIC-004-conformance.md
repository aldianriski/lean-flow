...
| [SPRINT-079](../../sprint/archive/SPRINT-079-the-undifferentiated-middle.md) | The Undifferentiated Middle | closed 2026-08-23 · `634e4b9` | **Took the exit condition's other half and removed the residual entirely, then covered two families on top.** The eleven `scope-out` rules that satisfied *neither* half of § Closed-when 2 — flagged here since SPRINT-073 — turned out to be correctly classified in the wrong **place**: a research register the engine cannot read, so all eleven reported to every adopter as `rule-unimplemented`, *checks the standard owes you and has not written yet*. §14 gains **`restated`** (7, each naming the rule that carries its constraint — §8's own answer one level down) and **`standard-directed`** (4 that govern this document, not a repository); **ADR-028**, and the cost is written into the condition rather than left to be noticed: **checkable 62 → 51 makes this condition easier to satisfy**, by a change made inside the epic that wants it ticked. Classification unchanged at **100**. Coverage **24 → 33** on top: §9's sprint-file family (5 rules, 6 findings, two reading git history) and §10's learning-governance family (4). §2 became addressable where §6 points — Multi-service's rows plus the rank-4 mapping the Tier cell had no branch for, and `DECISIONS.md` as a literal path, substrate-gated so it cannot demand itself from a repo with no decisions. **Every finding now names its rule** (23 of 54 verdict lines did not; appended rather than prefixed, because a prefix would have vacuously satisfied four guards that assert a finding's *absence*). **Three defects found by running the new checks on this repository**, which is what that is for: two §9 rules contradicting each other on this sprint's own Plan, an octal abort on `sprint: 079` that ended the report silently, and a fixture harness shipped unwired that the gate caught at close. **What it did not do:** § Closed-when 2 stays open at **39 of 51** — the remaining 12 `build` rules are `TASK-250/251/252`, exactly, with no residue. Six tasks (one added by amendment), 34 DoD, spec 0.5.0 → 0.8.0. |
| [SPRINT-080](../../sprint/archive/SPRINT-080-the-last-twelve-rules.md) | The Last Twelve Rules | closed 2026-08-23 · `309c0de` | **The epic's intended last, and it closed on the terms the condition already carried rather than on new ones.** The twelve outstanding `build` rules — §11's ledger-retention four, §11's archival four, §12's git-boundary four — verified by diff at promote to be **exactly** `TASK-250` + `TASK-251` + `TASK-252`, no residue either way. Coverage **39 → 51 of 51** and the register's § `build` bucket empty, after which § Closed-when 2 was **ruled** with both halves re-derived rather than carried forward: 45 checkable rules assert in the engine + 6 covered by standalone checkers each confirmed to emit verdict lines = **51 map to a check**; **49 explicitly marked** position-anchored from the spec (32 `judgment-only` · 7 `restated` · 6 `implementation-directed` · 4 `standard-directed`, no fifth mark); 51 + 49 = **100**, the full classified set. No amendment was made to reach the tick. **Two contributions that were not planned, both about checks that looked finished.** `check-epic-archive.sh` implemented **one half of §11's stated two-part test** — it had read `member_sprints` zero times across five sprints of green gates, and was wrong in both directions at once: it demanded an archive §11 forbids, and would have accepted an epic archived while a member sprint still ran, which is the silent case §11 names in its own text (→ **L-157**). And two controls shipped **vacuous**, caught only because a sibling must-FAIL went red; the fix is a shape rather than more diligence — a passing verdict now prints its own denominator (*"N shape-match(es) examined and cleared on content"*), so a control reporting **0 examined** is visibly untested instead of quietly green (→ **L-156**). **What it did not do:** the §12 four report **0 shape-matches against this repository**, so their content confirmations are exercised on fixtures alone; and the foreign-repo triage that § Closed-when 1 called *honest but early* is still un-re-run — now worth running against 45 rules rather than 6 (`TASK-238`). Six tasks, 34 DoD, gate 152/0. |
