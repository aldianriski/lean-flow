...
- [ ] `CLAUDE.md` and `CONTEXT.md` each carry ≥ 15% headroom against their caps — **`CLAUDE.md` 61/80
      = 24% ✓; `CONTEXT.md` 132/150 = 12%**, held at 150 by owner ruling (SPRINT-063 T1): a flat
      percentage is the wrong instrument for a file whose growth is measured at 0.83 lines/sprint and
      whose diet pass already found nothing removable (ADR-017). **Open pending an owner call on
      whether the condition is satisfied-as-ruled or wants re-wording** — not ticked unilaterally
