...
| [SPRINT-091](../sprint/SPRINT-091-full-run-and-first-family.md) | Full Run and the First Family | **active** — promoted 2026-08-27 | _(completed at close)_ Slice: **H12** (full orchestrator) · **H13** (F6 §4 ADR governance) · **H14** (fixture factories) · a pulled-forward slice of **H21**, targeting § Closed-when **2**. **The family-order open question closes here, with the profile it was parked for.** F6 was chosen on *both* axes — 2nd at real scale (72.1s) and sole owner of an always-on harness (30.0s, already spec-reduced to §4). **F11 was rejected despite ranking 1st** (84.7s): its only harness is `QA_FULL` opt-in, so migrating it moves the default gate by zero seconds — Round 6 having settled that the real-scale sweep is disjoint from the gate's leg 12, which runs a 7-rule reduced spec. Carries through to a converted harness and a **measured** delta rather than stopping at the family (D5 feature-first). Opens with a type checker: **TD-101** (`high`, open four sprints, never routed) records that nothing here evaluates a type, so ten TS tasks would satisfy type-level DoD with unchecked code. |
