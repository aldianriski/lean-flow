...
status: active
member_sprints: [SPRINT-083, SPRINT-085, SPRINT-087]
...
| [SPRINT-087](../sprint/SPRINT-087-first-rule-through-the-engine.md) | The First Rule Through the Engine | **active** — promoted 2026-08-25 | _(completed at close)_ Slice: H07–H11, targeting § Closed-when 2. **Feature-first by construction** — H07/H08/H09 are layers, so none is a task; every task crosses all three (D5). Tracer bullet first (one rule, every layer, differential parity against the Shell engine spawned as a live oracle), then rule-class resolution to `GAP`/`excluded`/`judgment-required` off ADR-036's six marks, then the first family whole rule-by-rule, then targeted `--rule`/`--section` with a partial run that emits **no** global level — the false-assurance shape this condition exists to prevent. Carries the three SPRINT-085 carry-forwards that unblock the moment a CLI exists. First family chosen at **G2**: cheap + representative per H10, with V3 §43 governing families 2..n. |
