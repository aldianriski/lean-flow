...
| [SPRINT-092](../sprint/SPRINT-092-the-conversions-measured-delta.md) | The Conversion's Measured Delta | **active** — promoted 2026-08-29 | _(completed at close)_ Slice: **H14** (fixture factories) · **H21** (the harness conversion) · the measured before/after § Closed-when **7** needs. **The half SPRINT-091 deferred by name.** 091 built the engine and migrated §4 whole while making the gate *no faster* — the saving and its evidence were held back to travel together, because a conversion shipped without its measurement is an unmeasured claim recorded as fact (D5). T2 converts the ADR-family harness to `bun:test` and drops it from the always-on eval leg; T3 relocates §4 differential parity to the opt-in profile under an ADR that names the **drift window** it opens and the moments parity MUST run; T4 measures the delta against SPRINT-091 T2's derived ceiling of **9.5–13.6 s** and **names any shortfall rather than smoothing it** (SPRINT-089 T1's precedent). Runs as the `engine` stream, concurrently with EPIC-015's `autonomy` stream; `scripts/qa-check.sh` and `evals/` are owned here, `gen-index.sh`/`knowledge-index.md`/`night-run.sh` there. **Not a cutover** — Shell retains §4 authority (D2), and **TD-120's git-spawn memoisation must land before H24–H26**, not here. |
