...
| [SPRINT-092](../sprint/archive/SPRINT-092-the-conversions-measured-delta.md) | The Conversion's Measured Delta | closed 2026-08-31 · `d43a7a1` | **Took §4's always-on coverage off the Shell engine and measured what that bought — including what it did not.** `run-s4-ts-evaluators.sh` replaces the 12-spawn harness in `eval_harnesses_always`: **23.4–28.2 s → 0.37–0.98 s**, with §4 rule semantics *and* the nine retained fixture directories still evaluating on every bare run, so this is a cost cut and not a coverage cut. The row-by-row TS-vs-Shell differential moved to opt-in under **ADR-039**, which names the drift window it opens and fixes parity as mandatory at promote, close and any full-profile run — **wired into `.claude/CONTEXT.md`, not into the shipped skills**, because hardcoding this repo's gate path into a consumer-installed skill is the L-015 leak. Fixture factories enforce H14's guardrail **at compile time** (a smuggled `expectedVerdict` does not type-check; an unused `@ts-expect-error` is itself an error, so `tsc --noEmit == 0` is a standing proof). **§ Closed-when 7 is deliberately NOT ticked.** Round 13 is a real before/after on one host with load stated, but it is **not the same workload**: the always-on leg no longer carries S4.APPEND's four git-building cases at all, so the 22.4–27.9 s saving beats Round 12's 9.5–13.6 s ceiling by ~2× *because it does less*, not because the conversion outperformed. The apples-to-apples test is `TASK-322`. Total work across both profiles went **up** ~76–85 s — the gate got faster by making the full profile slower, which is ADR-039's trade and now has figures. **`TD-117` did not close**: no clean whole-gate sample exists (default profile exceeds the 600 s command ceiling here; opt-in measures 1450 s), and `qa-budget-default` passes by comparing the *configured* budget to the ceiling rather than the runtime (**TD-128**); pruning 18 stale worktrees was tried as the cause and **disproved**, 1413 s → 1450 s. **The sprint self-verified green at 19/19 DoD and an independent worktree-isolated pass then found seven Tier G defects**, three of them single-line edits that remove real coverage while every guard the sprint built stays green — a harness passing with **zero tests run**, a parity regex blind to any prefixed call, and an `alwaysOn` flag compared only to a hardcoded copy of itself, all in code written to prevent exactly that. All fixed and re-proved on the reviewer's own seeds. `L-165` → `count: 4` · `L-184` · `L-185` · `TD-126`–`TD-128`. |
