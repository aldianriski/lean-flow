...
status: active
member_sprints: [workdoo SPRINT-001 (closed), workdoo SPRINT-002 (active)]
...
|---|---|---|---|
| [workdoo SPRINT-002](https://github.com/aldianriski/workdoo/blob/main/docs/sprint/SPRINT-002-driving-the-real-runtime.md) | Driving the Real Runtime | **active** — promoted 2026-09-09 · `28c5203` | _(completed at close)_ — the four proofs SPRINT-001 deliberately could not run: the `claude-code` adapter against a live CLI, a stop proven to terminate the **process** rather than the stream, a tool denial proven observable, and worker filesystem/network confinement with its gaps named. **A2 and A3 are both settled here**, and a broken A2 is recorded as an outcome rather than worked around |
