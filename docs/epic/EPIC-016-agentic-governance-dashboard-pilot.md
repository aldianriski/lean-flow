...
status: active
member_sprints: [workdoo SPRINT-001 (closed), workdoo SPRINT-002 (closed), workdoo SPRINT-003 (closed), workdoo SPRINT-004 (closed), workdoo SPRINT-005 (closed), workdoo SPRINT-006 (closed), workdoo SPRINT-007 (active)]
...
|---|---|---|---|
| [workdoo SPRINT-007](https://github.com/aldianriski/workdoo/blob/main/docs/sprint/SPRINT-007-something-to-bind-an-approval-to.md) | Something to Bind an Approval To | **active** — promoted 2026-09-20 | _(completed at close)_ — **phase 3 foundations: the three things an approval needs before it can refuse anything.** The Inbox has been blocked for three sprints on one fact — a run’s work product is deleted with its worktree, so an approval has nothing to bind to (`TD-017`). `ADR-003` ruled where that record lives; T2 builds it as a per-attempt Git ref whose sha the store records. T3 gives `attempts.verification_state` a writer: the column has carried a check constraint since `0002` and every row is null, so F05’s “an absent verdict must hold” currently guards a value nothing produces. T4 is F02 made real — an approval bound to a resolved digest, with a different revision refused and a stale one refused, each saying which it was — preceded by T1 writing `ADR-004` first, because a binding rule discovered while coding the binding is how an approval comes to be bound to a label. The Inbox screen itself is deliberately deferred: it depends on both T2 and T4, and pulling it would put the sprint’s pace on a serial chain |
