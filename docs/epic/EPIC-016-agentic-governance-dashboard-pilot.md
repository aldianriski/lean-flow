...
status: active
member_sprints: [workdoo SPRINT-001 (closed), workdoo SPRINT-002 (closed), workdoo SPRINT-003 (active)]
...
|---|---|---|---|
| [workdoo SPRINT-003](https://github.com/aldianriski/workdoo/blob/main/docs/sprint/SPRINT-003-a-real-run-from-the-browser.md) | A Real Run From the Browser | **active** — promoted 2026-09-14 · `efdf3aa` | _(completed at close)_ — roadmap phase 2: an operator creates a task in a browser, a real run starts, activity is visible as it happens, and a stop is confirmed, with nothing lost across a browser restart. Eight tasks — the ADR-002 ruling first, because it may amend the runtime port everything downstream reads, then persistence · owner session auth (F01's minimum action: the grant is server-issued, never asserted by the caller) · the plan→implement→verify workflow with a capability issued per phase · the supervisor dispatching one **separate-process** worker under lease and heartbeat · live activity that replays from the store · a stop carrying SPRINT-002's OS-confirmed `processGone` · deployment. **Deliberately eight against a 2–5 guideline**: deployment was kept in-sprint by owner decision and chains back through the whole phase, so `T8` is ordered last and **blocked on `TASK-010`** — the sprint can exhaust seven tasks and park only its tail. `T5` is the bottleneck and says so |
