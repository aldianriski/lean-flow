...
| [SPRINT-107](../sprint/archive/SPRINT-107-retarget-the-loop.md) | Retarget the loop | closed `cef7344` | The loop runs on the store. Sprints reference their members and `plan_commit` is the freeze (ADR-047, a Tier G checker that cleared three outside rounds). `/triage`, `/task-decomposer`, `/handoff`, `/flow` and `/orchestrator` read and write task files; merge-back is coordinator-owned with a duplicate-id check. Closed-when **1** (37-task breakdown → 37 files, no cap) and **5** (derived progress = hand count) met. |
