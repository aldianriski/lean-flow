...
status: active
member_sprints: [072, 073, 074, 075, 076, 077, 078]
...
| [SPRINT-078](../sprint/SPRINT-078-the-checks-a-stranger-cannot-see.md) | The Checks a Stranger Cannot See | **active** · promoted 2026-08-21 | *In progress — row completed at close.* Aims at § Closed-when 2's coverage half: five §13 rules that already have working, fixture-guarded checks are **invisible to an adopter** (`conformance.sh` execs the engine alone; `check-attestation.sh` is reached only from `qa-check.sh`), so migrating them is reach rather than new assertion code; plus §2/§6's tier doc-set family as one check with the tier a parameter, and §2's README ownership footer. Coverage **19 → 30 of 62** if all three land. Does **not** touch the ruling on the 11 `scope-out` rules (TASK-254), which is the condition's other half. |
