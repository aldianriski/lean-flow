...
| [SPRINT-076](../sprint/SPRINT-076-coverage-and-the-bar.md) | Coverage, and Whether the Bar Is Right | active | _(completed at close)_ — the sprint that makes this epic's remaining bar judgeable instead of guessable. Five tasks: the per-check fixture audit that closes **§ Closed-when 3** or writes down why it cannot · the §4 ADR family (five mechanical rules against a 27-ADR corpus) · §2's placement pair, chosen **because** it is the likeliest artefact source rather than the safest · a ruling on **§ Closed-when 2** taken last, with T1–T3's numbers in hand · and §3's two exceptions that are enforced in code today but absent from the standard. **T4 is the one that must not run early**: its whole value is deciding with evidence, and amending an exit condition to fit what got built is L-088's failure. Sized down deliberately to 20 DoD after SPRINT-075's 26 breached the Plan cap. **Gates not yet signed.** |
