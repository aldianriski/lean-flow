...
| [SPRINT-098](../sprint/SPRINT-098-prove-the-run-then-report-it.md) | Prove the Run, Then Report It | **active** — promoted 2026-09-11 · `a341378` | **Targets § Closed-when 1 · 5 · 6 — the three this epic cannot close with a guard.** T1 first removes the reason SPRINT-093 left CW 1 untickable *and* the reason nobody would have noticed: `check-night-run-rollup.sh` is reachable only through the reaper, which fires on unattended runs, so the attended mode this repository actually runs in has no rollup guard at all (L-192 · L-166's mode-axis sibling). T2 ships the bounded repair loop at exactly ADR-022's ceiling (CW 5) and T3 the typed run outcome with its nine evidence fields (CW 6), with the EPIC-015-vs-EPIC-008 ownership of that vocabulary ruled **before** a `RunSummary` shape is minted rather than after both epics have one. T4 is the run itself, carried by `TASK-319` from SPRINT-093's own close — and **D3 pre-locks that it is not gated on T2/T3 being green**, because SPRINT-060 foreclosed this exact task's only vehicle by letting an unrelated G2 ruling decide the run's shape (L-111). T5 pairs `TASK-188` to it so that if the run stops mid-Plan, someone is positioned to claim the artifact instead of spending it. Contribution completed at close. |
