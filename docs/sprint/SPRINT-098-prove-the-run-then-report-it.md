...
- [x] The **continuation contract** moves out of `orchestrator/SKILL.md` step 4's paragraph into its own headed section at G1/G2's structural level, naming the five terminal states (L-192 — form, not wording). — ✓ own `## Continuation contract` section naming all five terminal states; SKILL.md 129/140
...
- [x] The outcome is **wired into what reads the log** — the template's event vocabulary and the rollup consumer, not present only in its own file (L-020). — ✓ template event vocabulary + `check-night-run-rollup.sh` + `night-run.md` Part 4. Five fields carry **no** consumer check, stated as a scope decision rather than read as full wiring
