...
**DoD:**
- [x] Spawn counts are cut in the families T1 confirms as dominant — *Verify: per-family spawn counts re-measured, appended as a round rather than edited in* — **reinterpreted by owner ruling (ADR-021), recorded in the `scope-change` entry**: T1 confirmed the dominant cost of the *default* profile is **leg 12 (396.3s of 492s)**, not Round 5's F11/F6/F5/F9 (which cost 1.9–5s there and 281.2s only under `QA_FULL=1`). Ticked against leg 12's harnesses. Not a silent re-read — DoD 3 was unachievable via the frozen target
- [x] **No check deleted, no coverage lowered** — *Verify: the check inventory before and after is diffed and identical; a quieter gate is the shape of a check that stopped running (observed: 183 → 168 at SPRINT-085's close, which reconciled exactly)* ✓ coordinator-verified statically from **three independent angles, all agreeing**: 50 unique fixture names before and after with **zero removed and zero added**, assertion count unchanged, and no fixture/target/assertion touched. (The runtime before/after capture was killed mid-run, so the static evidence is what carries this — stated rather than glossed)
- [x] The gate completes **under load**, not only on a clean table — *Verify: a run issued after substantial agent work in the same session prints its verdict line; a pristine-table run does not satisfy this because it already passes* ✓ **met in the coordinator session, on a genuinely loaded table** — 6 live worktrees, 7 agent dispatches, 4 prior full gate runs, all recorded before the run. Result: **226 lines, `QA-CHECK: 176 pass, 7 fail` printed**. SPRINT-085's three attempts under comparable load died at 204 / 117 / 100 lines **without ever printing a verdict**. **The nuance, stated rather than smoothed:** it completed *because* T3's budget guard tripped at 461s and cut the run short, naming its three skipped harnesses — which is the designed behaviour, not a loophole. Printing a verdict with named skips is exactly what "completes" was written to mean, and the alternative it replaced was dying mute (TD-084)
