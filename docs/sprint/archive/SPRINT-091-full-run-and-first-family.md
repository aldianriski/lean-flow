...
Layers: `apps/cli/src/` · `main.ts` · `main.test.ts` (the composition site and its tests — both genuinely touched by `e5d59ce`, declared here per L-100 rather than left to the directory token, which the checker does not read as covering a bare basename)
Depends-on: T6, T7
...
**DoD:**
- [x] All five §4 evaluators dispatch through `composedDispatch` — ✓ run live by the coordinator and again by the reviewer: `--section 4 .` reports **zero `rule-unimplemented`** and all five evaluate for real (PASS across this repo's 38 ADRs). The control half holds too — S4.BAR and S4.NOINVENT still render `excluded/judgment-required`, unchanged, so the wiring reached exactly the rows it should and no others
- [x] The laundering case is closed, proven on the artifact that motivated it — ✓ and **proven against the defect, not merely against a test**: the reviewer checked `main.ts` out at `e5d59ce^` and reproduced the old behaviour on the same repro dir (zero FAIL lines, `level: Attested`), then re-ran post-fix. TS now reports `FAIL S4.INDEX` where it gapped, and `level: none` against live-spawned Shell's `level: none`. **The level-VALUE half is recorded as weaker than the per-rule half, deliberately:** Shell independently FAILs S2.F-FILE (×5) and S6.BASE (×2) on this fixture, so Shell would print `none` whether or not §4 worked — that match is over-determined. It is the **per-rule S4.INDEX FAIL↔FAIL agreement that carries the proof**, and on the TS side the Attested→none flip is cleanly attributable to T12 because S4.INDEX is its *only* Structural FAIL there. Written down rather than smoothed, because a criterion that reads satisfied for the wrong reason is the failure this sprint keeps finding (L-136)
