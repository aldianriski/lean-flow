...
- [ ] The gate completes **under load**, not only on a clean table — *Verify: a run issued after substantial agent work in the same session prints its verdict line; a pristine-table run does not satisfy this because it already passes* — **UNMET, and deliberately not substituted.** T2 started a background gate run to capture the under-load verdict; it was killed before producing output. A clean-table run would satisfy the words and prove nothing, since that case already passes today. Recorded as an open gap in § Round 7 Caveats. **Closable only in a loaded session** — this one qualifies (six agents dispatched)
