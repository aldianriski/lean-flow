...
**DoD:**
- [ ] The frozen surface names all seven semantic elements and disclaims byte-exact stdout — *Verify: read-through against V3 §25's two lists; every element in the freeze list present, every element in the do-not-freeze list absent*
- [ ] The rule-ID denominator is **derived**, and the three disagreeing counts (100 / 79 / 51) are each accounted for — which is the contract's, and what the other two are counting — *Verify: the ADR states each number's source command and why it differs; a bare assertion of one number is a fail*
- [ ] A snapshot of the frozen rule IDs is committed under `evals/fixtures/compat/`, stamped with `spec/STANDARD.md`'s `version:` (0.10.0 today) — *Verify: regenerating from `sh scripts/lib/read-spec-rules.sh spec/STANDARD.md` reproduces the snapshot byte-for-byte (`cmp`)*
- [ ] Finding IDs are enumerated from the live engine, not from memory — *Verify: the list is produced by a command recorded in the ADR, and a second, differently-shaped query agrees on the count (CLAUDE.md cross-check)*
