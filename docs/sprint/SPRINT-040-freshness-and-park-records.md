...
**DoD:**
- [ ] `skills/prime/SKILL.md` gains the check: compare the base-dir version from the skill's own
      invocation header against `.claude-plugin/plugin.json`; `version:` frontmatter bumped
- [ ] Output format shows the row in all three shapes: `fresh` · `STALE — reinstall before trusting
      any procedure` · `n/a (no local plugin repo)`
- [ ] **STALE branch demonstrated firing** against a manifest whose version differs — a freshness row
      that can only ever print `fresh` is exactly the silent false-negative L-058 names
- [ ] `n/a` branch demonstrated on a repo with no `.claude-plugin/plugin.json` (consumer path, L-015)
- [ ] `fresh` branch demonstrated on this repo as-is
- [ ] README `/prime` blurb + CHANGELOG entry reflect the user-visible change (L-015)
- [ ] TD-015 marked `status: resolved → SPRINT-040 T1` with the accepted blind spot stated
