...
- [ ] `promote` stamps `sprint:` and **moves** files rather than rendering task content into the sprint file; `close` verifies members are in `done/`/`cancel/` and writes the Retro. The sprint Plan remains an explicitly approved snapshot **by reference** — the freeze must survive the change, not be traded away for it. Transitions are coordinator-owned with a duplicate-id check at merge (D6): two isolated worktrees can each claim one ticket from their own snapshot.
