...
| `S12.SECRETS` · `S12.BACKUPS` · `S12.DESIGNSRC` · `S12.GENERATED` | `conformance-engine.sh` *(SPRINT-080 T3 — §12's shape-detectable four, and the riskiest in the epic: the register warns that a filename heuristic flagging `contract.md` in a contract-testing repo is **worse than no scan**. So a finding needs **two signals that agree** — a shape (extension / filename / path) and a confirmation read from content or git state. **Refused, on the record:** size thresholds for BACKUPS and DESIGNSRC, because §12 says "large" and "small" and states no number, and a figure written into the checker is a threshold the standard never set (L-097); replaced by signals §12 does state — a dump-tool preamble, and the asset directories it names by path. Bare filename matching refused throughout. §12c's classes and its one `MAY be committed` carve-out are both **read from the spec**, and the carve-out is **subtracted** rather than positionally trimmed — it is named before the permission in the same sentence, so trimming keeps it and would fire on the one file §12 allows (L-140). **Eleven retained cases, six of them controls, and the controls were built FIRST**: `.env.example`, a `.pem` holding only a public CERTIFICATE, a small fake `seed.sql`, `public/hero.mp4`, a shared `.vscode/extensions.json`, and a `.env` of placeholders only — every one a shape a heuristic would flag)*|
...
**Round 4 (SPRINT-081 T3) and every later round live in [`logs/conformance-coverage.md`](logs/conformance-coverage.md)** — the series split to its uncapped sibling at 129 of 130 lines (§2 `research/logs/`). Verdict at 45 rules: **9 findings, 9 actionable, 0 artefacts**, reconciled three ways — **re-run at Round 6** (SPRINT-100 T5, TD-089) after the harness half of it was found to sweep only 6 of those 9 FAIL lines, blind to the `S<N>.<CODE>` finding convention. Verdict unmoved, now measured over 9 of 9.
