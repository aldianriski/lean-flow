...
> **Question.** `03-ADLC-ROADMAP.md` names phases A–H and two ordering blocks, but nothing maps them onto
> epics, dependencies or lanes. Which are epics, what must be sequential, what can run at the same time?
> **Verdict.** Two lanes plus a research side-car. Phases B and M1 are already open as EPIC-005 and
> EPIC-006 and are structurally disjoint, so they run concurrently. Phase E is mis-sequenced late and is
> pulled forward as EPIC-007; Phase D becomes EPIC-008, gated on EPIC-006. Phases F, G, M5 and H get
> **register rows with named admission conditions**, not epic files — the roadmap's own anti-goal forbids
> platform abstractions before real consumers exist.

## Why this matters

`platform-readiness-audit.md` sequenced EPIC-002…005 and held 20 sprints; it has run out — EPIC-004 closed
2026-08-23 and nothing past Phase B is costed. The failure mode tested for is not picking the wrong
phase, it is running phases **serially that have no dependency on each other**.
...
Adjustment` orders *Run Protocol → Plugin Event Emission*; `03` Phase D, the strategy `README.md` and
`EPIC-006` D2 order it the other way. → **RULED: emission first, protocol derived** (owner, 2026-08-24),
guard into EPIC-008 D1 — an object with no observed event shape is excluded from v1.

**F2 — A second contradiction, now RULED.** `03 § Roadmap Adjustment` placed Memory/Context/Cost **after**
the gateway; the strategy `README` **before** dashboard observability. Neither: **after Connected Workspace,
before the Gateway** — EPIC-011 (owner alignment, 2026-08-24). A judgement call closed by ruling, as L-094
requires, rather than parked for a measurement that was never coming.

**F3 — EPIC-005 and EPIC-006 are disjoint except for two files.** Fleet touches `conformance.sh`,
`scripts/lib/` and `mktemp -d` fixture repos (D3); Run Evidence touches emission call-sites in `skills/`, a
record format and a reader. Overlap is `spec/STANDARD.md` (§2 · §11) and `.claude/CONTEXT.md` → two lanes,
**`spec/` owned by Lane 1** — the cross-stream rule applied before the collision, not after.

**F4 — Phase C is unstarted, unblocked, absent from the backlog.** `docs/research/harness-delta.md` does not
exist, and a census for its four candidate names returns zero hits across `docs/`, `spec/` and `skills/`;
`harness-engineering-adaptation.md` predates the pack. Phase C feeds D, collides with nothing: the long pole.

**F5 — Phase E is mis-sequenced.** `03 § 5` requires *"at least 2 runtime **or workflow** variations prove
abstraction pressure"* — workflow packs satisfy it, depend only on the closed Standard, and Phase E says to
start from packs with real internal use, which the 14 skills are. Listing it after Phase D leaves a **gate
condition blocking that has no blocker of its own**. → EPIC-007, queued behind 006 (both edit `skills/`).

**F6 — The platform decision gate is 2 of 6, every open condition now owned.**
...
Lane 2   EPIC-006 Evidence ─▶ EPIC-007 Packs ─▶ EPIC-008 Protocol ┤  009 = pre-gate,
side-car Phase C harness-delta ─────────────────▶ feeds 008       │  read-only, no
            └─▶ EPIC-009 Local Shadow Observability ◀── 006       │  control authority
...
**The dashboard is five rows, not one.** `08 § 16`'s MVPs are different outcomes: MVP0 is read-only emission (M1); MVP1 is bidirectional — dashboard creates and assigns Work Items, plugin syncs back, a Human Inbox exists (M2). An earlier draft folded them, which `06 § 3` warns against in as many words: *"do not jump directly to Managed Mode."*
...
**One prerequisite, found while costing this and now closed:** EPIC-005's pin-and-upgrade mechanic had no rule for what an upgrade may break, while `spec/` shipped nine MINOR bumps in eight days. Ruled → **ADR-032** · `spec/` §15 · spec 0.10.0 · EPIC-005 D4.

### The core execution architecture is FROZEN (SPRINT-082)

The register above gates new *scope*; this gates new *workflow*. It sits here because this is the file
read when an epic is proposed — a freeze recorded anywhere else governs nothing (L-151). The Gauntlet
loop — gates · dispatch · review routing · revise · system verify · run protocol — is **existing
architecture, not backlog**; "a better workflow idea" is not an admission condition. A change is admitted
**only** on a **measured defect** · a **measured cost** · a **repeated workflow failure** (≥2 sightings,
`L-NNN`) · a **security issue** · **consumer evidence** from an adopter who is not us. All three classes
of fact are named on purpose (L-094) — a *measurement* accumulates, a *documented behaviour* is closed by
reading, a *judgement call* by ruling — because gating on "a measurable signal" alone parks the latter
...
- **Platform repository boundary** — where dashboard/gateway/adapters live *as source*, undecided and owed
  before EPIC-009/010. One `lean-flow/` growing `dashboard/ gateway/ database/ worker-runtime/` is the
  fastest route to dev-flow 2.0; the split is **lean-flow** (Standard · plugin · conformance · protocol)
  vs **lean-platform** (gateway · control plane · dashboard · adapters · persistence). → ADR at 009 G2.
- Whether F1's emission-first ruling is ADR-grade → likely (§4). **Offered, not written:** EPIC-006's
  first sprint opens with a superseding ADR for ADR-013 (c), which may carry both rulings.
