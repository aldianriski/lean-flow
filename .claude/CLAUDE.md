...
❌ Edit-safety — three traps where **the tool reported success and the artifact was wrong**: **(a)** `git add <shared-file>` while another stream/task has WIP in it — stages their uncommitted work into your commit (contaminates at the **commit** phase → mis-attributed, hard-to-reverse history); serialize the stream, or `git add -p <shared>` + verify `git diff --cached` first (L-042 · cf. L-037). **(b)** Trusting a **structure-adjacent Edit** — a markdown table-row / list-entry edit can silently fuse or corrupt neighboring rows/entries (grep + line caps stay clean); re-read the whole structure after the edit — a fresh-context review catches what the author can't (L-009 ×3: table-row fusion Sprint-007 · mid-table insert Sprint-028 · TASK-006 heading fused in TODO.md). **(c)** Trusting a command's **self-report** — an exit code or a reply channel is evidence about the *reporter*, never about the artifact: a gate piped into a formatter returns the formatter's status · a failed redirect reports non-zero before the gate ever ran · `commit` stores a mangled subject at exit 0 · a killed subagent's reply says failure over valid output already on disk. A non-zero status with no report behind it is not a verdict, and a zero one is not a pass — inspect the thing the command was supposed to produce (the file, the stored text, the gate's own output), and for a fan-out treat the per-unit output **file** as the success signal (L-045 · L-049 · L-057 · L-059 · L-060 — 5× across 4 sprints).
