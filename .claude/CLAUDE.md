...
skills/              # 14 SKILL.md files (plugin auto-discovers)
  ...
  lean-doc-generator/
    ...
    templates/*.md.template       # 25 canonical doc templates (core, incl. SPRINT/ADR/RESEARCH/DEPLOYMENT/ROLLBACK/BUG/TECH-DEBT/CONTRIBUTING/SECURITY/AGENTS/database-{erd,schema,migration-guide}/architecture-{data-flow,authentication,integrations}; +2 non-core: DESIGN·QA-TESTCASE = 27 total)
...
❌ Edit-safety — two silent-corruption traps: **(a)** `git add <shared-file>` while another stream/task has WIP in it — stages their uncommitted work into your commit (contaminates at the **commit** phase → mis-attributed, hard-to-reverse history); serialize the stream, or `git add -p <shared>` + verify `git diff --cached` first (L-042 · cf. L-037). **(b)** Trusting a **structure-adjacent Edit** — a markdown table-row / list-entry edit can silently fuse or corrupt neighboring rows/entries (grep + line caps stay clean); re-read the whole structure after the edit — a fresh-context review catches what the author can't (L-009 ×3: table-row fusion Sprint-007 · mid-table insert Sprint-028 · TASK-006 heading fused in TODO.md).
