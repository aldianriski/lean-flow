...
| `sprint-bulk` | G1+G2 once | auto-loop the active sprint's members |
...
- [ ] Each `done-when` notes its **verification method** where a mechanical check exists — that named check's FAIL then blocks the *silent* tick: surface → recorded owner ruling, never tick past it quietly (ADR-021)
- [ ] **Reachability — for every mechanical `Verify:`, four questions, not one.** **EXISTS** (is the method actually there?) · **RUNS** (in the target environment?) · **REACHES** (does its scope touch the artifact or behaviour the criterion claims?) · **PROVES** (if it PASSes, is the criterion true?). *A check that never examines the target is not verification.* Naming a method is the half that gets checked today and the half that fails safely; an **unreachable** criterion reads exactly like a satisfied one — it goes green while saying nothing about its subject (L-136 ×4: a `Verify:` naming `check-doc-caps.sh` for `docs/qa/` files, where §2 states no cap for `docs/qa/`, so it could neither pass nor fail them and ran `66 PASS, 0 FAIL`). **A method whose scope excludes the target is recorded as not-valid-proof, not accepted.** Where no mechanical method reaches it, the criterion stays a **judgment tick and says so** — that is legitimate, and inventing a checker merely to make a criterion look mechanical is the failure, not the fix. EXISTS and REACHES are mechanically pre-screened (`scripts/lib/check-verify-reaches.sh`); RUNS and PROVES are yours
- [ ] **Heading for an `overnight` run?** Record the **approval envelope** in the sprint frontmatter — `approval_envelope: goal · scope · acceptance · design · verification · j1-delegation · capabilities · repair-policy · budget · stop-conditions @ <sha>`. Gates say *the Plan is sound*; the envelope says *the run may proceed inside these bounds without asking* — different grants, so signing G1/G2 does not imply one. All ten, or the envelope can silently widen along the one omitted; frontmatter, never the transcript, because that is the only thing an unattended run reads (L-099 · L-151). Absence means NOT approved (`references/night-run.md` Part 1a step 4b)
- [ ] **Authority class declared per task — `J0` · `J1` · `J2`**, in the header meta beside `class:` (`references/night-run.md` Part 0 § Authority classes). J0 needs no approval · J1 is delegated in advance by a recorded pre-launch approval and runs unattended **inside that envelope only** · J2 is human-reserved and **parks**. **Declared, never inferred, and an absent class reads as J2** — the safe end, because an undeclared class is an unasked question and an unasked question is a BLOCK. Against `HITL`/`AFK` it is a one-way implication, not independence — `J2` ⇒ `HITL` always, `HITL` ⇏ `J2`: staffing describes the run, the class describes the task, so a J1 task run with a human present is still J1
...
## Continuation contract — when an unattended run may stop

`references/night-run.md` Part 0 says what a run may **do**; this says when it may **stop**. A run
does not pause between tasks the owner already approved — a `J1` task is one the recorded approval
covers, so re-confirming it re-asks a settled question; finishing a task is not a decision point. It
**ends at exactly one of five terminal states, named in the rollup**: `PLAN_EXHAUSTED` (every task
reached a resolved state) · `AUTHORITY_BOUNDARY` (work remains, all of it J2 or blocked behind a
park) · `HARD_FAILURE` (a step the run cannot proceed past) · `BUDGET_STOP` (a declared ceiling
reached with Plan remaining) · `USER_STOP` (a human interrupted). Anything else is a stop nobody
declared — the failure that reads as success, since a turn that simply ends carries `subtype:
success` and no error. The launcher writes the state, as it already writes the DoD count (ADR-016) —
this changes *when* a run stops, never *who* records it. Full contract → `references/night-run.md` Part 0b.
...
### sprint-bulk
Operates on the active sprint file `docs/sprint/SPRINT-NNN-<slug>.md` and its **members by reference** (ADR-047): the `## Members` paths ∪ every `docs/work/*/TASK-*.md` stamped `sprint: SPRINT-NNN`, each resolved by id in whatever status folder it sits.
A member's DoD is its own `## Done when`; § Plan carries only sprint-scoped meta (`Layers:` · `Depends-on:` · `Cites:`), never DoD. Resolution · tick · transitions · close → `${CLAUDE_SKILL_DIR}/references/dispatch.md` § Members by reference.
0. **Guard** — verify an active sprint (a top-level `docs/sprint/SPRINT-*.md` with `status: active`, never `logs/` or `archive/`) with a member still open — an open `## Done when` `[ ]`, or not yet in `done/`/`cancel/`. No active sprint → halt, redirect to `/lean-doc-generator promote`; every member closed → step 6. More than one active (parallel streams) → ask which sprint to run.
1. **Batch G1** — one combined scope pass over the Plan.
2. **Batch G2** — one design pass; **map shared-file ownership** (every file touched by >1 task → single owner + commit order, before the first task) and note cross-task file overlaps — and **cross-stream** ones: files shared with another stream's active sprint → coordinate or sequence, never parallel-build; at commit, stage shared files per-hunk (`git add -p` + verify `git diff --cached`) — a plain `git add <shared>` stages another task/stream's WIP into your commit (contaminates at the commit phase, not just merge — promoted rule); **grill individually any task with an unconfirmed `assumes:`** — a batch sign-off never waves an open assumption through. **The map is built from `Layers:`, so it cannot see files no task declares** — the sprint file (§ Files Changed), its Execution Log, and every member file's ticks and `git mv` transitions are written across tasks and owned by the **coordinator**, never assigned to one; a dispatched agent returns its Log entry and evidence in its report instead of writing them (SPRINT-063 produced two copies of one Log → `${CLAUDE_SKILL_DIR}/references/dispatch.md` § Worktree dispatch protocol).
...
4. **Loop** — per Plan task (`### Tn`, delivering the member(s) its `Cites:` names): `git mv` the member `todo/`→`in_progress/` → Implement (route by type — new behaviour → `/tdd`) → Self-review → Commit → tick the **member file's** `## Done when` box `[x]` (` ✓ <evidence>` suffix) → `git mv` it on to `review/`/`done/`, each move in its own commit (ADR-045 D6);
   **append to the sprint Execution Log** (never edit § Plan — it's frozen and holds no DoD). `/loop` can pace it; unattended overnight run → `${CLAUDE_SKILL_DIR}/references/night-run.md` (pre-flight + trigger + **the unattended contract, Part 0**). **Run the loop until the Plan is exhausted — every member's boxes ticked or its task carrying a rollup line — then emit the rollup block at exit, always, headed by `run · N of M DoD ticked`, counted over the members' `## Done when` boxes** (night-run.md Part 4). Nothing outside this loop checks the Plan is finished, so a turn that ends mid-Plan ends the session reporting `success`; the count is what makes that visible (`unattempted` is its state). **Governed by the Continuation contract above** — do not pause between already-approved tasks, and end at exactly one of its five named terminal states.
...
6. **Close** — every member in `docs/work/done/` or `cancel/` (the host's `check-sprint-by-reference.ts <sprint> --close` gate where it has one, else the plain-git check in dispatch.md § Members by reference)
   → **first, once, after the final wave's merge-back, run system-verify** (the discovered host gate against the integrated tree; a FAIL blocks this close per ADR-021 → `${CLAUDE_SKILL_DIR}/references/dispatch.md` § System verify) → then `/lean-doc-generator close`; then **fixes-only sprint → `/release-patch` (PATCH); feature sprint → MINOR by hand** (release-patch is PATCH-only, scans `plan_commit..HEAD`).
...
❌ **Silently absorbing a mid-sprint scope change** — a pivot that shifts scope is logged (`scope-change`: what broke · impact · re-confirm G2) in the Execution Log *before* editing the frozen § Plan or a member's `## Done when` (SPRINT-012 T4 · ADR-047).
