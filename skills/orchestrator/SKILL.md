...
0. **Guard** — verify an active sprint (a top-level `docs/sprint/SPRINT-*.md` with `status: active`, never `logs/` or `archive/`) whose members have an open `## Done when` `[ ]` exists. None → halt, redirect to `/lean-doc-generator promote`. More than one active (parallel streams) → ask which sprint to run.
