...
| User says… | Do |
|---|---|
| "promote" / "start sprint" | **Layout check gates entry (v1/mixed stops here, § above).** **Governance review first** (below) → **size-check what you are about to pull: a `[size: L]` task is split (or sent back to `/task-decomposer`) BEFORE it is rendered, never after.** G1 already splits an `L` — but G1 runs *after* `promote` has rendered, frozen and committed the Plan as `plan locked`, so the split then costs a `scope-change` entry plus a Plan amendment against a commit minutes old. At pull time it costs nothing → pull chosen `state: ready` Backlog tasks (TODO.md, dependency order) into a new `docs/sprint/SPRINT-NNN-<slug>.md` rendered from `templates/SPRINT.md.template` (each task → a Plan `Tn` with DoD checkboxes); set `status: active` + `plan_commit`; **once the batch G1/G2 pass is signed, record it as `gates_signed: G1,G2 @ <sha>` in the sprint frontmatter — an unattended run reads the sprint file and nothing else, so a sign-off living only in the launching session's transcript is invisible to it and every task parks having done no work (L-099). Omit the field until the gates are actually signed: its absence means NOT signed, and must never be read as approval**; **if the tasks advance an open epic, set `epic: EPIC-NNN` in frontmatter and append the sprint's row to that epic's § Member sprints** (ask which epic only when it is ambiguous — never guess one); point the stream's pointer in TODO.md § Active Sprint at the file (single-stream: the lone pointer; **multi-stream: confirm which stream this sprint belongs to before writing `stream:`**); commit `sprint(N): plan locked` |
