...
`promote` / `close` / epic rollup refuse v1 or mixed by name and point to `/lean-doc-generator migrate` — the only 2.x path accepting one; doc/ADR generation and `migrate` itself are unaffected.

## Migrate (adopt + clean existing docs)

Align an existing docs tree (dev-flow · adlc-flow · ad-hoc · **v1 work-item layout**) to lean-flow
...
AskUserQuestion popup; docs + the safe-scaffold allowlist only, never `settings.json`. **Headless
parks to `/handoff`.** Procedure · tier table · allowlist → `${CLAUDE_SKILL_DIR}/references/init.md`.
...
| User says… | Do |
|---|---|
| "promote" / "start sprint" | **Layout check gates entry (v1/mixed stops here, § above).** **Governance review first** (below) → **size-check what you are about to pull: a `[size: L]` task is split (or sent back to `/task-decomposer`) BEFORE it is rendered, never after.** G1 already splits an `L` — but G1 runs *after* `promote` has rendered, frozen and committed the Plan as `plan locked`, so the split then costs a `scope-change` entry plus a Plan amendment against a commit minutes old. At pull time it costs nothing → promote **by reference** (`references/sprint-by-reference.md`): `git mv` the chosen `state: ready` tasks `docs/work/backlog/ → todo/` in their own commit, stamp `sprint:`, render a new `docs/sprint/SPRINT-NNN-<slug>.md` from `templates/SPRINT.md.template` — `## Members` lists the files; each Plan `Tn` holds sprint-scoped meta only, **never a DoD copy**; set `status: active` + `plan_commit` (**the freeze** — that reference says what it pins); **once the batch G1/G2 pass is signed, record it as `gates_signed: G1,G2 @ <sha>` in the sprint frontmatter — an unattended run reads the sprint file and nothing else, so a sign-off living only in the launching session's transcript is invisible to it and every task parks having done no work (L-099). Omit the field until the gates are actually signed: its absence means NOT signed, and must never be read as approval**; **if the tasks advance an open epic, set `epic: EPIC-NNN` in frontmatter and append the sprint's row to that epic's § Member sprints** (ask which epic only when it is ambiguous — never guess one); point the stream's pointer in TODO.md § Active Sprint at the file (single-stream: the lone pointer; **multi-stream: confirm which stream this sprint belongs to before writing `stream:`**); commit `sprint(N): plan locked` |
| executing during a sprint | Tick the member file's `## Done when` `[x]` as each passes (rewording it needs a `scope-change` Log entry naming the id — the freeze); **append to the Execution Log sibling (`docs/sprint/logs/`, rendered lazily from `templates/sprint-log.md.template` at the first entry), never edit § Plan** (the plan is frozen); keep Files Changed current |
| "close" / "sprint done" | **Layout check gates entry (v1/mixed stops here, § above).** Verify every member sits in `docs/work/done/` or `cancel/` and the freeze holds — never count Plan boxes (`references/sprint-by-reference.md` § Close by reference); **sweep the full session** (Execution Log + any TD/follow-up surfaced mid-run but not yet filed) for the buckets; write the **Retro** + route its buckets (§10); set `status: closed` + `close_commit`; clear that stream's pointer in TODO § Active Sprint; **run §11 retention as one propose→approve pass** — apply only on owner approval: **archival pass** (move closed sprint → `docs/sprint/archive/` **and its log → `docs/sprint/archive/logs/` in the same commit** + a line in `docs/sprint/INDEX.md` · **if a rolled-up epic now has every member sprint closed AND every § Closed when `[x]`, move it → `docs/epic/archive/` and keep its `INDEX.md` row, re-basing its relative links one level deeper** — never on member-sprint count alone (§11) · remove shipped tasks' Backlog entries outright, no shipped-in comments · scrub remaining TODO.md refs to the closed SPRINT-NNN outside § Active Sprint · verify CHANGELOG rotation links resolve) + **compaction sweep** (periodic, same gate: promoted `L-NNN` bodies → one-line pointers; superseded/duplicated research → set `status: superseded`, then move → `docs/research/archive/` **only once nothing live still cites it** (§11); measured line delta reported); **doc-freshness check** — map Files Changed against §2 update triggers and propose refreshes for affected docs (propose→approve, never silent); squash-commit `sprint(N): <summary>`; then **fixes-only → `/release-patch` (PATCH) · feature sprint → MINOR by hand** (release-patch is PATCH-only). **If the sprint carries `epic:`, roll up before committing**: complete its row in that epic's § Member sprints (status · `close_commit` · *what it contributed*, not a restatement of the theme), re-check the epic's § Closed when, and close the epic only when **every** condition is `[x]` — a member sprint closing is not an epic closing. **The rollup is checked, not trusted** — an active epic whose member closed without a row, whose row carries no `close_commit`, or whose header now predates that close, is drift a promote-time check reports (§ Governance review) |
...
**Handoff reconciliation gates close (STANDARD §12(b)).** Sweep the Execution Log for any `handoff`
entry not yet `spent`; route each item to a durable home (or rule it needs none) and append a
`spent` entry sharing its `handoff-path` — a close-time check FAILs over one still `live`/`consumed`.
Full protocol → `references/handoff-reconciliation.md`.
...
`☐ L-promotion (count≥2, promoted:no): <findings|none>` · `☐ TD aging (≥3 sprints unaddressed): <findings|none>` · `☐ doc-aging — §11 retention (TD deletion · CHANGELOG rotation · LEARNINGS pointer-collapse · TODO prune) **+ every §2 cap breach**: <findings|none>`. · `☐ epic rollup currency (every `status: active` epic): <findings|none>` · `☐ handoff ledger (root HANDOFF-LEDGER.md, live/consumed rows): <findings|none>`
**Doc-aging reads from two sources and neither is this list.** §11's ledger owns *retention*; §2's table owns *caps*. Report every §11 trigger that fired **and** every §2 breach — sourced from the project's own cap check where it has one, otherwise measured directly against §2's table. **Never restate a §2 cap figure here**: the enumeration above is a routing hint, not the source, and a copied number is a second SSOT that drifts from the row it copied. This is the failure that made the line worth fixing — a cap check printed three soft breaches on every run for sprints while this review reported doc-aging clean, because the checklist enumerated §11's triggers and a §2 breach was not among them (a matcher with no consumer — SPRINT-062 T2 · L-106).
**Epic rollup currency is a THIRD source and is not doc-aging.** Retention asks *should this epic be
archived?*, a question that only goes live at the end; this asks *is an active epic's rollup current?*
Three drift classes, each its own finding: a closed member sprint with no rollup row — or a row
carrying no `close_commit` — in § Member sprints; an ownership header whose `last_updated` predates its
own trigger's last firing (for an epic, the newest member close); a ticked § Closed-when condition
naming no closing sprint. All three are invisible to a schema check, which asserts a field is
**present** and never that it is **current** — which is how an epic sits misreporting itself through a
green gate for sprints (SPRINT-094 T1).
