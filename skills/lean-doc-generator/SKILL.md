...
`☐ L-promotion (count≥2, promoted:no): <findings|none>` · `☐ TD aging (≥3 sprints unaddressed): <findings|none>` · `☐ doc-aging — §11 retention (TD deletion · CHANGELOG rotation · LEARNINGS pointer-collapse · TODO prune) **+ every §2 cap breach**: <findings|none>`. · `☐ epic rollup currency (every `status: active` epic): <findings|none>`
