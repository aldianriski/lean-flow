...
<br />**14 skills · a repo scaffolder (`/lean-doc-generator init`) · no hooks and no agent definitions in the roster today.** Drop it into any repo and adapt.
...
- No hooks and no agent definitions in the roster today — the loop leans on Claude's built-in agents (`Explore`, `/code-review`, `/verify`, `/security-review`) rather than re-shipping them. Neither is banned: **ADR-044** holds both to ADR-001's curation bar, and the first hook candidate was withdrawn at review rather than shipped (see `TASK-366`). The one skill that orchestrates sub-agents internally is the opt-in `/council`.
...
- **No hooks or agent definitions in the roster today; leverages the built-in agents** — the loop dispatches Claude's built-in agents (`Explore` · `/code-review` · `/verify` · `/security-review`) in isolated passes. `/council` is the one skill that orchestrates sub-agents internally.
