...
| `adr/ADR-NNN-<slug>.md` | medium+ | Team | per file, append-only | a qualifying decision (§4) | never edited in place — superseded / deprecated by a later ADR |
| `DECISIONS.md` (the ADR index; under `docs/`) | medium+ | Team | append-only | with the first ADR | new ADR → index row |
...
| **Medium / complex** | multi-dev, sustained, or architecturally forked | + `adr/` · `flows/` (CHANGELOG is already always-core — ADR-012; the index's own presence is asserted by S4.INDEX, which fires once an ADR exists) — plus **substrate-conditional**, skipped not owed when the substrate is absent: `DECISIONS` (a qualifying decision has been taken — §2 says don't pre-create it before the first real entry) |
