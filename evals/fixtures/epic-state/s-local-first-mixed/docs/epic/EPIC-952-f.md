...
the incidental property nobody chose: it discriminates the two `|| continue` skips and is structurally
blind to `report_unresolvable`'s `&& continue`, which only a LOCAL-FIRST list can reach.
