---
name: orchjudge
description: Use when starting, resuming, or completing any development task or sprint. Drives a gate-driven loop — quick, mvp, and sprint-bulk modes — with a G1 Scope gate and a G2 Design gate before any commit. Self-contained, no specialist agents.
...
4. **Loop** — per Plan task: Implement (route by type — new behaviour → `/tdd`) → Self-review → Commit → tick its DoD `[x]`; **append to the sprint Execution Log** (don't edit § Plan — it's frozen). `/loop` can pace it; unattended overnight run → `${CLAUDE_SKILL_DIR}/references/night-run.md` (pre-flight + trigger + **the unattended contract, Part 0**).
5. **First-blocker halt** — stop on any blocker or human `block`; log it and wait. **Unattended** (the trigger said so — never inferred): don't wait — **use your best judgement and carry the task through to completion** (note the decision → Execution Log), the same as any other AFK-safe step. Charter is execute-to-completion: a flagged step is judged and finished, never stalled on or left for a human who isn't there. Contract → `night-run.md` Part 0 (v1.22 amendment: Continuation policy).
...
❌ **Stalling an unattended run on an unreachable human** — headless has no ask channel at all (`AskUserQuestion` unregistered; `dontAsk` auto-denies); an unattended run that idles waiting for input which cannot arrive has failed its one job (finish the night's work). Use best judgement, note the decision, and keep the run moving — that is the autonomy `sprint-bulk unattended` grants, not a licence to sit idle (v1.22 amendment, `night-run.md` Part 0).
